...
for g in a.split('\n')[:-1]:
	...
	for p,i in enumerate(b.split('\n')[:-1]):
		#for i in b.split('\n')[:-1]:
			...
			c = os.popen( 'ls /imatge/mcompri/Desktop/THESIS/VGG_16/traininig_data_generator/').read()
			for q,idx in enumerate(c.split('\n')[:-1]):
				l=os.path.splitext(idx)[0]
				word = ['hola','agricultural','airplane',"baseballdiamond","beach","buildings","chaparral","denseresidential","forest","freeway","golfcourse","harbor","intersection","mediumresidential","mobilehomepark","overpass","parkinglot","river","runway","sparseresidential","storagetanks","tenniscourt"]
...
# ricorda di copiare i dati dal file dataset.txt in dataset.csv per aggiornare le labels
...
#model.fit_generator(...)
model.fit(trainFeatures,r,batch_size=40,validation_split=0.2,nb_epoch=10,verbose=2)

print("saving model and weights")
model_json = model.to_json()
with open("ResNet50_prova.json", "w") as json_file:
	json_file.write(model_json)
print("saving...")
model.save_weights('resnet50_weights_prova.h5')
